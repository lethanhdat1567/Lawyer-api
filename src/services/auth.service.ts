...
/** Sau đăng ký — chưa cấp JWT; user cần verify email. */
export type RegisterResponse = {
  user: PublicUser;
  message: string;
};

/** Verify email: có session chỉ khi vừa xác minh lần đầu. */
export type VerifyEmailResult =
  | (AuthTokensResponse & { verified: true; message: string })
  | { verified: true; message: string };

export async function ensureUniqueUsername(baseRaw: string): Promise<string> {
  const prisma = getPrisma();
  const sanitized =
    baseRaw.slice(0, 24).replace(/[^a-zA-Z0-9_]/g, "_") || "user";
...
export async function registerUser(input: {
  ...
  return {
    ...
    message: AuthCopy.REGISTER_CHECK_EMAIL,
  };
}
...
export async function verifyEmailWithToken(
  ...
  if (row.user.emailVerifiedAt) {
    return { verified: true, message: AuthCopy.EMAIL_VERIFIED };
  }
...
export async function signInWithFirebase(
  ...
  if (userByEmail) {
    const conflicting = userByEmail.authIdentities.some(
      (i) =>
        i.provider === AuthProvider.FIREBASE && i.providerUserId !== uid,
    );
    if (conflicting) {
  ...
  const username = await ensureUniqueUsername(email.split("@")[0] || "user");
